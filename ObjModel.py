...
from magic import Mat3, Mat4, build_shader, get_uniform_location_debug
...
class ObjModel:
    ...
    def __init__(self, fileName):
        ...
        self.defaultShader = build_shader(self.defaultVertexShader, self.defaultFragmentShader, self.getDefaultAttributeBindings())
    ...
    def render(self, shaderProgram = None, renderFlags = None, transforms = {}):
        ...
        for tfmName,tfm in defaultTfms.items():
            loc = get_uniform_location_debug(shaderProgram, tfmName)
        ...
        for material, chunkOffset, chunkCount, renderFlags in chunks:
            ...
            if material != previousMaterial:
                ...
                for k,v in material["color"].items():
                    glUniform3fv(get_uniform_location_debug(shaderProgram, "material_%s_color"%k), 1, v)
                glUniform1f(get_uniform_location_debug(shaderProgram, "material_specular_exponent"), material["specularExponent"])
                glUniform1f(get_uniform_location_debug(shaderProgram, "material_alpha"), material["alpha"])
    ...
    def setDefaultUniformBindings(self, shaderProgram):
        ...
        glUniform1i(get_uniform_location_debug(shaderProgram, "diffuse_texture"), self.TU_Diffuse);
        glUniform1i(get_uniform_location_debug(shaderProgram, "opacity_texture"), self.TU_Opacity);
        glUniform1i(get_uniform_location_debug(shaderProgram, "specular_texture"), self.TU_Specular);
        glUniform1i(get_uniform_location_debug(shaderProgram, "normal_texture"), self.TU_Normal);
